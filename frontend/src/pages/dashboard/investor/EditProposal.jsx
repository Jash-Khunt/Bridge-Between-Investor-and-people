...
const EditProposal = () => {
    ...
    const fetchProposal = async () => {
      ...
      if (data) {
        setFormData({
          ...
          investmentRange: data.investmentRange || { min: "", max: "" },
  ...
  const handleSectorChange = (sector) => {
    const updated = formData.sectorsOfInterest.includes(sector)
      ? formData.sectorsOfInterest.filter((s) => s !== sector)
      : [...formData.sectorsOfInterest, sector];

    setFormData({ ...formData, sectorsOfInterest: updated });
  ...
  const validateForm = () => {
    ...
    if (!formData.investmentRange.min) {
      ...
      newErrors["investmentRange.min"] = "Minimum must be a positive number";
    }
    ...
    if (!formData.investmentRange.max) {
      ...
      newErrors["investmentRange.max"] = "Maximum must be a positive number";
    } else if (
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await updateProposal(id, formData);
    if (result.success) {
      navigate("/dashboard/my-proposals");
    } else {
      setErrors({ general: result.message || "Failed to update proposal" });
    }
  };
      ...
      <div className="rounded-lg bg-white p-6 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end space-x-3 border-t border-gray-200 pt-6">
            ...
            >
              {isUpdating ? <LoadingSpinner size="sm" /> : "Update Proposal"}
            </button>
          </div>
        </form>
      </div>
