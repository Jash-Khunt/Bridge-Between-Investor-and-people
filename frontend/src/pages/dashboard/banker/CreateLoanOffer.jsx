...
const CreateLoanOffer = () => {
  ...
  const { createLoanOffer, isCreating } = useLoanOfferStore();
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        const formattedInterestRate = formData.interestRate.endsWith("%")
          ? formData.interestRate.slice(0, -1)
        ...
        setErrors({
          ...errors,
          general: "Failed to create loan offer. Please try again.",
        });
      }
    }
  };
    ...
    <div className="container mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Create a new loan product to attract potential borrowers
        </p>
      </div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end space-x-3 border-t border-gray-200 pt-6">
            ...
            >
              {isCreating ? <LoadingSpinner size="sm" /> : "Create Loan Offer"}
            </button>
          </div>
        </form>
      </div>
